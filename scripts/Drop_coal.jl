...
# ╔═╡ 95b13a64-0b24-443f-8031-8925243f8c3b
using Swalbe, DataFrames, CairoMakie

# ╔═╡ cf61fc70-2fee-11ec-3aea-6f486f27ded7
md"# Droplet Coalescence

This is an example use case for **Swalbe.jl**.
We would like to study the coalescence of two droplets next to each other.
Due to surface tension any given liquid tries to minimize its surface area.
That is why droplets are found in the shape of a spherical cap or a sphere.

To model this problem we can use the thin film equation 

$$\frac{\partial h}{\partial t} = \nabla\cdot(M(h)\nabla p)$$

## Loading depencies

First we need to load a few dependencies.
Since this is a showcase of **Swalbe.jl** we need to import it.
For visualization either of the packages, **Plots.jl** or **Makie.jl** is fine.
Makie has generally nice plots, but it is a little bit harder to use.
Plots is the default plotting package.
For data export and import we use **DataFrames.jl**, **JLD2.jl** and **CSV.jl**.

"

# ╔═╡ c9192fb1-0422-428b-8009-49a59543db09
md"## Initial condition

The starting point of coalesence study is the height field of two droplets.
Therefore we want an initial condition that places to spherical cap shaped droplets next to each other, barely in contact."

# ╔═╡ 1ff01b96-bfc3-418d-aac9-88d686433d50
"""
	twodroplets(L; r₁=230, r₂=230, θ₁=1/9, θ₂=1/9, center=(L/3,2L/3))

A not so clever way for generation of the height profile of two droplets
"""
...
# ╔═╡ c10f3e64-162d-4bf5-bb7c-1363d2fad5bf
begin
	# The two droplet profile
	h0 = twodroplets(1000);
	# The plot, starting with x and y data, figure details and axis labeling
	lines(1:1000, h0,
	figure = (resolution = (700,450), fontsize = 18), 
	axis = (xlabel = L"l [Δx]", ylabel = L"h", backgroundcolor = :white))
	
	current_figure()
end

# ╔═╡ 83d608e8-ac34-4908-a61b-39eeea3c11e4
"""
    run_drop_coal()
...
git-tree-sha1 = "28fb63760e7ae7006e3236358dee43dd74dd462d"
