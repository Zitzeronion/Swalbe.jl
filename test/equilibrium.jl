@testset "Equilibria" begin
    ...
    @testset "Density and velocity" begin
        ...
        #active struct
        state = Swalbe.Sys(Swalbe.SysConstActive(Lx=5, Ly=5), "CPU")
        state.feq .= 0.0
        state.height .= 1.0 
        state.velx .= 0.1 
        state.vely .= -0.1
...
@testset "Equilibria 1D" begin
    ...
    @testset "Density and velocity" begin
        ...
        @test all(state.geq[:,1] .== 1.0 - 0.01)
        @test all(state.geq[:,2] .≈  0.5 * 0.1 + 0.5 * 0.01)
        @test all(state.geq[:,3] .≈  -0.5 * 0.1 + 0.5 * 0.01)
        
        # Deprecated
        # state.geq .= 0.0
        # state.rho .= 0.0
        # state.rho_vel .= 0.1
        # Swalbe.rho_equilibrium_linear!(state, sys)
        # @test all(state.geq[:,1] .== 0)
        # @test all(state.geq[:,2] .≈ 0)
        # @test all(state.geq[:,3] .≈  0)
