...
@testset "Capillary pressure Active" begin
    # Struct
    sys = Swalbe.SysConstActive(Lx=5, Ly=5, n=3, m=2, γ_0=1.0, hmin=0.1, hcrit=0.1, θ_0=0.0)
    state = Swalbe.Sys(sys, "CPU")
    state.height .= reshape(collect(1.0:25),5,5)
    # With arguments struct
    f = reshape(collect(1.0:25),5,5)
    ...
    @testset "No height gradient" begin
        nograd = ones(5,5)
        state.height .= 1.0
        sys2 = Swalbe.SysConstActive(Lx=5, Ly=5, n=3, m=2, γ_0=1.0, hmin=0.1, hcrit=0.0, θ_0=1/2)
        Swalbe.filmpressure!(res, nograd, 1.0, 1/2, 3, 2, 0.1, 0.0)
        Swalbe.filmpressure!(state, sys2)
        for i in eachindex(res)
            @test res[i] .≈ -2(0.1^2-0.1) atol=1e-10
...
@testset "Capillary pressure Active 1D" begin
    # Struct
    sys = Swalbe.SysConstActive_1D(L=30, n=3, m=2, γ_0=1.0, hmin=0.1, hcrit=0.1, θ_0=0.0)
    state = Swalbe.Sys(sys)
    state.height .= collect(1.0:30)
    sys2 = Swalbe.SysConstActive_1D(L=30, n=3, m=2, γ_0=1.0, hmin=0.1, hcrit=0.0, θ_0=1/2)
    state2 = Swalbe.Sys(sys2)
    # Without the struct
    f = collect(1.0:30)
    f_float = collect(1.0f0:30.0f0)
    ...
    @testset "No height gradient" begin
        ...
        state2.height .= 1.0
        Swalbe.filmpressure!(res, nograd, dummy, 1.0, 1/2, 3, 2, 0.1, 0.0)
        Swalbe.filmpressure_fast!(state2, sys2)
        @test res[15] ≈ -2(0.1^2-0.1) atol=1e-10
        @test state2.pressure[15] ≈ -2(0.1^2-0.1) atol=1e-10
    ...
    @testset "Gradient and contact angle" begin
        Swalbe.filmpressure!(res, f, dummy, 1.0, 1/2, 3, 2, 0.1, 0.0)
        Swalbe.filmpressure_fast!(state, sys2)
        sol = zeros(30)
        sol[1] = 30
        sol[end] = -30
        for i in eachindex(res)
