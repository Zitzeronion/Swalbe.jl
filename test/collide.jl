@testset "Collision and Stream" begin
    ...
    @testset "Dummy dists τ=0.75 no forces" begin
        ...
        #With active struct
        sys = Swalbe.SysConstActive(Lx=5, Ly=5, τ=0.75)
        state = Swalbe.Sys(sys, "CPU")
        state.feq .= 1.0
        state.ftemp .= 1.0
        state.fout .= 1.0
    ...
    @testset "Dummy dists τ=1 with forces" begin
        ...
        # With active struct
        sys = Swalbe.SysConstActive(Lx=5, Ly=5)
        state = Swalbe.Sys(sys, "CPU")
        state.feq .= 1.0
        state.ftemp .= 1.0
        state.fout .= 1.0
    ...
    @testset "Dummy dists τ=1 with forces 1D" begin
        ...
        @test all(state.gout[:,1] .== state.geq[:,1])
        @test all(state.gout[:,2] .== circshift(state.geq[:,1] .+ 1/20, 1))
        @test all(state.gout[:,3] .== circshift(state.geq[:,1] .- 1/20, -1))
    end
    @testset "Dummy dists τ=0.75 with forces 1D" begin
        ...
        sys = Swalbe.SysConstActive_1D(L=30, τ=0.75)
        state = Swalbe.Sys(sys)
        state.ftemp .= 1.0
        state.fout .= 1.0
        state.feq .= 1.0
        ...
        omega = 1.0 - onebytau
        Swalbe.rho_BGKandStream!(state, sys)
        @test all(state.gout[:,1] .== omega .* 1.0 .+ onebytau .* state.geq[:,1])
        @test all(state.gout[:,2] .== circshift(omega .* 1.0 .+ onebytau * state.geq[:,1].+ 1/20,1))
        @test all(state.gout[:,3] .== circshift(omega .* 1.0 .+ onebytau * state.geq[:,1].- 1/20,-1))
    end
end
