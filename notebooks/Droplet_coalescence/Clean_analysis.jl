...
# ╔═╡ 61661178-9dcb-4a0c-83d7-f747335c3aa6
using DataFrames, CSV, DataFramesMeta, Plots, Swalbe, LaTeXStrings, FileIO, ImageMagick

# ╔═╡ 83309542-caea-11ec-389c-f5e8be899125
md"# To *coalesce* or not to *coalesce*
A story about two attracted droplets which can not become one

Sessile droplets which are in contact with each other will always try to minimize their energy.
Identical droplets will therefore coalesce to form a single droplet with less surface area than the sum of the two.
This is a well studied problem, on different wettabilities, droplet size, complex liquids.

Is it always true that two sessile droplets coalesce, you may ask?

**No** it is not, there are scenarios in which although in contact with each other they ultimately descide to stay seperated.
With a somewhat challenging path to seperation.
"

# ╔═╡ 28bca160-280e-4d8c-bdbc-f3103f5ab7a6
md"First we need some data, luckily everything is precomputed and we can just load a csv file."

# ╔═╡ 49521b00-e4d8-477f-a07b-e7952df0c38c
begin
	data_location = "..\\..\\data\\coalescence_analysed\\bridge_data_combined.csv"
	df_coalescence = CSV.read(data_location, DataFrame)
end

# ╔═╡ 23112ad7-c94c-4de6-8490-71aa695c0b58
md"This data frame has 101000 rows with 7 columns, all of it is data related to droplet coalescence experiments.
The meaning of the different columns is simply
1. gamma: The kind of surface tension gradient with options
   - const: Simply a constant surface tension, the base case
   - step: A step function at the middel of the domain
   - tanhX: Smoothing with a tangent hyperbolicus
2. time: Lattice Boltzmann time step
3. bridge_min: The minimum thickness value in the bridge region (around the center)
4. bridge_max: The maximum thickness ...
5. neck_pos: Position of the minimum on the lattice
6. neck_max_pos: Position of the maximum in the bridge region
7. skewness: A measure for the asymmetry of the coalescence process

Here we use the term bridge region ($\mathcal{BR}$), which is just an interval on the lattice

```math 
\mathcal{BR} = \bigg[\frac{L-R_0}{2}, \frac{L+R_0}{2}\bigg]
``` 

where $R_0$ is the radius of either one of the droplets at $t=0$.
Below we plot the initial setup of the experiment, keep in mind we work in the lubrication limit and thus with small contact angles.
The scaling of the y to x axis is 1:7.
"

# ╔═╡ 3b30115e-fd2f-4b22-a8fa-3d85ae13fa74
begin
	...
	vspan!([512-171, 512+171]; alpha = 0.3, label = L"\mathcal{BR}")
	ylims!(0,40)
	xlims!(0,1024)
end

# ╔═╡ cbd27b46-a987-4c63-9bff-454ee20bf1c7
begin
	t10 = collect(10:10:1000)
	t1000 = collect(2000:1000:10000000)
	t1k = collect(1000:1000:10000000)
	tr = vcat(t10, t1000)
end
...
md"Time is an elusive thing, there is never enough when it is needed and plenty to spend when there is nothing to do.
The measure of time, *lattice Boltzmann time steps* is not really relateable to experiments.
That is why we introduce a meaningful time scale, the *interio-capillary time* given by the equation
...
md"So how does this graph look for different γ(x) values?
Let's plot it!"
...
begin
	...
	# Data
	plot(tr ./ tau_ic(γ=0.0001), 
		sort!(@subset(df_coalescence, :gamma .== "const", :slip .== sl_here), :time).bridge_min ./ 171,
		l=(3, :solid),
		label="const", 
		axis=:log,
		xlabel="t/τ",
		...
		legend=:topleft)
	plot!(tr ./ tau_ic(γ=0.0001), 
		sort!(@subset(df_coalescence, :gamma .== "step", :slip .== sl_here), :time).bridge_min ./ 171,
		l=(3, :auto),
		label="step")
	plot!(tr ./ tau_ic(γ=0.0001), 
		sort!(@subset(df_coalescence, :gamma .== "tanh2", :slip .== sl_here), :time).bridge_min ./ 171,
		l=(3, :auto),
		label="tanh2")
	plot!(tr ./ tau_ic(γ=0.0001), 
		sort!(@subset(df_coalescence, :gamma .== "tanh10", :slip .== sl_here), :time).bridge_min ./ 171,
		l=(3, :auto),
		label="tanh10")
	plot!(tr ./ tau_ic(γ=0.0001), 
		sort!(@subset(df_coalescence, :gamma .== "tanh50", :slip .== sl_here), :time).bridge_min ./ 171,
		l=(3, :auto),
		label="tanh50")
	plot!(tr ./ tau_ic(γ=0.0001), 
		sort!(@subset(df_coalescence, :gamma .== "tanh200", :slip .== sl_here), :time).bridge_min ./ 171,
		l=(3, :auto),
		label="tanh200")
	# Fit
...
git-tree-sha1 = "31b1e379d7aa71f4234587c6d94a1d95471c8086"
